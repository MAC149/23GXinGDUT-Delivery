...
{      	
	...
	else if(fonts==32)
	{
		for(j=0;j<4;j++)
		{
			OLED_SetCursor((Line - 1) *4+j, (Column - 1) * 16);		//
			for (i = 0; i < 16; i++)
			{
				OLED_WriteData(OLED_F16x32[Char - ' '][i+j*16]);		
			}
		}
	}
	else if(fonts==164)
	{
		for(j=0;j<8;j++)
		{
			OLED_SetCursor(j, (Column - 1) * 16);		//设置光标位置在上半部分
			for (i = 0; i < 16; i++)
			{
				OLED_WriteData(OLED_F16x64[Char - ' '][i+j*16]);		
			}
		}
	}
}
...
void OLED_ShowString(uint8_t Line, uint8_t Column, char *String,uint8_t fonts)
{
	...
	{
		OLED_ShowChar(Line, Column + i, String[i],fonts);
	}
}
...
void OLED_ShowNum(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length,uint8_t fonts)
{
	...
	for (i = 0; i < Length; i++)							
	{
		OLED_ShowChar(Line, Column + i, Number / OLED_Pow(10, Length - i - 1) % 10 + '0',fonts);
	}
}
...
void OLED_ShowSignedNum(uint8_t Line, uint8_t Column, int32_t Number, uint8_t Length,uint8_t fonts)
{
	...
	if (Number >= 0)
	{
		OLED_ShowChar(Line, Column, '+',fonts);
		Number1 = Number;
	}
	else
	{
	...
	for (i = 0; i < Length; i++)							
	{
		OLED_ShowChar(Line, Column + i + 1, Number1 / OLED_Pow(10, Length - i - 1) % 10 + '0',fonts);
	}
}
...
void OLED_ShowHexNum(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length,uint8_t fonts)
{
	...
	for (i = 0; i < Length; i++)							
	{
		...
		if (SingleNumber < 10)
		{
			OLED_ShowChar(Line, Column + i, SingleNumber + '0',fonts);
		}
		else
		{
			OLED_ShowChar(Line, Column + i, SingleNumber - 10 + 'A',fonts);
		}
	}
}
...
void OLED_ShowBinNum(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length,uint8_t fonts)
{
	...
	for (i = 0; i < Length; i++)							
	{
		OLED_ShowChar(Line, Column + i, Number / OLED_Pow(2, Length - i - 1) % 2 + '0',fonts);
	}
}
