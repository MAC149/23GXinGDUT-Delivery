...
void OPS_init(void)
{
	__HAL_UART_ENABLE_IT(&huart1,UART_IT_IDLE);                         //使能串口3的空闲中断
	HAL_UART_Receive_DMA(&huart1,OPS.pucRxbuff,opsRxbuff_LENGTH); //串口3开启DMA接收
//	GUI_ShowCHinese(0,0,16,"车子正在初始化中",1);
	HAL_Delay(7000);
	HAL_Delay(6000);
//	Update_all();
//	OLED_Clear(0);
